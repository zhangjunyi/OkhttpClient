...
public final class HttpEngine {
	 ...
	 * @param connection
	 *            the connection used for an intermediate response immediately
	 *            prior to this request/response pair, such as a same-host
	 *            redirect. This engine assumes ownership of the connection and
	 *            must release it when it is unneeded.
	 * @param routeSelector
	 *            the route selector used for a failed attempt immediately
	 *            preceding this
	 */
	public HttpEngine(OkHttpClient client, Request request,
			boolean bufferRequestBody, boolean callerWritesRequestBody,
			boolean forWebSocket, Connection connection,
			RouteSelector routeSelector, RetryableSink requestBodyOut,
	 ...
	 *             Sometimes recoverable. See {@link #recover(RouteException)}.
	 * @throws IOException
	...
	private void connect() throws RequestException, RouteException {
		...
		if (routeSelector == null) {
			...
			try {
				routeSelector = RouteSelector.get(address, networkRequest,
						client);
			} catch (IOException e) {
	...
	private Connection createNextConnection() throws RouteException {
		...
		for (Connection pooled; (pooled = pool.get(address)) != null;) {
			if (networkRequest.method().equals("GET")
					|| Internal.instance.isReadable(pooled)) {
				return pooled;
			}
	...
	/**
	 * Attempt to recover from failure to connect via a route. Returns a new
	 * HTTP engine that should be used for the retry if there are other routes
	 * to try, or null if there are no more routes to try.
	 */
	public HttpEngine recover(RouteException e) {
		...
		if (routeSelector == null && connection == null // No connection.
				|| routeSelector != null && !routeSelector.hasNext() // No more
																		// routes
																		// to
																		// attempt.
				|| !isRecoverable(e)) {
		...
		return new HttpEngine(client, userRequest, bufferRequestBody,
				callerWritesRequestBody, forWebSocket, connection,
	...
	private boolean isRecoverable(RouteException e) {
		...
		// Problems with a route may mean the connection can be retried with a
		// new route, or may
		// indicate a client-side or server-side issue that should not be
		// retried. To tell, we must look
		// at the cause.
	...
	private void connectFailed(RouteSelector routeSelector, IOException e) {
		// If this is a recycled connection, don't count its failure against the
		// route.
	...
	/**
	 * Release any resources held by this engine. If a connection is still held
	 * by this engine, it is returned.
	 */
	public Connection close() {
		...
		// If this engine never achieved a response body, its connection cannot
		// be reused.
		if (userResponse == null) {
			if (connection != null)
				closeQuietly(connection.getSocket()); // TODO: does this break
														// SPDY?
			connection = null;
			return null;
		}

		// Close the response body. This will recycle the connection if it is
		// eligible.
		closeQuietly(userResponse.body());

		// Close the connection if it cannot be reused.
		if (transport != null && connection != null
				&& !transport.canReuseConnection()) {
			closeQuietly(connection.getSocket());
			connection = null;
			return null;
		}

		// Prevent this engine from disconnecting a connection it no longer
		// owns.
	...
	private Request networkRequest(Request request) throws IOException {
		...
		if (cookieHandler != null) {
			// Capture the request headers added so far so that they can be
			// offered to the CookieHandler.
			// This is mostly to stay close to the RI; it is unlikely any of the
			// headers above would
			// affect cookie choice besides "Host".
			Map<String, List<String>> headers = OkHeaders.toMultimap(result
					.build().headers(), null);
	...
	class NetworkInterceptorChain implements Interceptor.Chain {
		...
		public Response proceed(Request request) throws IOException {
			...
			if (permitsRequestBody(request) && request.body() != null) {
				Sink requestBodyOut = transport.createRequestBody(request,
	...
	private Response readNetworkResponse() throws IOException {
		...
		Response networkResponse = transport
				.readResponseHeaders()
		...
		if (!forWebSocket) {
			networkResponse = networkResponse.newBuilder()
					.body(transport.openResponseBody(networkResponse)).build();
		}
	...
	public void receiveHeaders(Headers headers) throws IOException {
		CookieHandler cookieHandler = client.getCookieHandler();
		if (cookieHandler != null) {
			cookieHandler.put(userRequest.uri(),
					OkHeaders.toMultimap(headers, null));
		}
	}
	...
	public Request followUpRequest() throws IOException {
		if (userResponse == null)
			throw new IllegalStateException();
		Proxy selectedProxy = getRoute() != null ? getRoute().getProxy()
				: client.getProxy();
		...
		case HTTP_UNAUTHORIZED:
			return OkHeaders.processAuthHeader(client.getAuthenticator(),
	...
	private static Address createAddress(OkHttpClient client, Request request) {
		...
		return new Address(request.httpUrl().host(), request.httpUrl().port(),
				client.getSocketFactory(), sslSocketFactory, hostnameVerifier,
				certificatePinner, client.getAuthenticator(),
