 ...
 * shared - once shared they should be treated as immutable and can safely be
 * used to concurrently open new connections. If required, threads can call
...
public class OkHttpClient implements Cloneable {
	...
	static {
		Internal.instance = new Internal() {
			@Override
			public Transport newTransport(Connection connection,
			...
			@Override
			public void closeIfOwnedBy(Connection connection, Object owner)
					throws IOException {
			...
			@Override
			public void setCache(OkHttpClient client,
			...
			@Override
			public void connectAndSetOwner(OkHttpClient client,
					Connection connection, HttpEngine owner, Request request)
	...
	private OkHttpClient(OkHttpClient okHttpClient) {
		...
		this.internalCache = cache != null ? cache.internalCache
 ...
 */
	...
	/**
	 * Sets the socket factory used to create connections. OkHttp only uses the
	 * parameterless {@link SocketFactory#createSocket() createSocket()} method
	 * to create unconnected sockets. Overriding this method, e. g., allows the
	 * socket to be bound to a specific local address.
	 * 
	 * <p>
	 ...
	 * {@link #setSslSocketFactory SSL socket factory} to establish trust.
	 ...
	 * 
	 * <p>
	 * If unset, the {@link java.net.Authenticator#setDefault system-wide
	 * default} authenticator will be used.
	 */
	 ...
	 * 
	 * <p>
	 * If unset, the {@link ConnectionPool#getDefault() system-wide default}
	 ...
	 * 
	 * <p>
	 * If unset, protocol redirects will be followed. This is different than the
	 ...
	 * encountered. By default, this client silently recovers from the following
	 * problems:
	 * 
	 * <ul>
	 * <li><strong>Unreachable IP addresses.</strong> If the URL's host has
	 * multiple IP addresses, failure to reach any individual IP address doesn't
	 * fail the overall request. This can increase availability of multi-homed
	 * services.
	 * <li><strong>Stale pooled connections.</strong> The {@link ConnectionPool}
	 * reuses sockets to decrease request latency, but these connections will
	 * occasionally time out.
	 * <li><strong>Unreachable proxy servers.</strong> A {@link ProxySelector}
	 * can be used to attempt multiple proxy servers in sequence, eventually
	 ...
	 * only call this method to avoid specific compatibility problems, such as
	 * web servers that behave incorrectly when SPDY is enabled.
	 * 
	 * <p>
	 ...
	 * 
	 * <p>
	 * <strong>This is an evolving set.</strong> Future releases include support
	 * for transitional protocols. The http/1.1 transport will never be dropped.
	 * 
	 * <p>
	 ...
	 * 
	 * @param protocols
	 *            the protocols to use, in order of preference. The list must
	 *            contain {@link Protocol#HTTP_1_1}. It must not contain null or
	...
	/**
	 * Returns a modifiable list of interceptors that observe the full span of
	...
	/**
	 * Cancels all scheduled or in-flight calls tagged with {@code tag}.
	 ...
	 * accessible to HTTP clients as {@link SSLSocketFactory#getDefault()}. If
	 * we used the shared SSL context, when OkHttp enables ALPN for its
	 * SPDY-related stuff, it would also enable ALPN for other usages, which
	 * might crash them because ALPN is enabled when it isn't expected to be.
	 * 
	 * <p>
	 * This code avoids that by defaulting to an OkHttp-created SSL context. The
	 * drawback of this approach is that apps that customize the global SSL
